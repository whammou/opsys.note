...
* Document Drafts :typDoc:catDraft:OPSYS:meta:
#+HTML:</details>
...
** OUTL Notification Forwarding :subNotification:
...
** OUTL Systemd power supply detection :subSystemd:
...
** OUTL Podman Quadlet :subPodman:containerization:
...
** OUTL Switching to Wayland Guide :subWayland:GUIDE:
:PROPERTIES:
...
3.  Start =fail2ban= systemd service:
    ...
    #+END_SRC
** DOIN Homelab Remote Access Via VPN :subVPN:subWireGuard:subHomelab:
...
-  Gateway Peer
** OUTL Screen-saver and Screen-lock :subWayland:MONITOR:
