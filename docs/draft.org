...
#+FILETAGS: :topOpsys:typDoc:catDraft:
* OUTL [#A] CPU and IRQ scheduler :PERFORMANCE:
...
* OUTL [#A] Select io scheduler :PERFORMANCE:
- /Change/ ~udev~ /rules for io-scheduler/
* OUTL [#A] Disable Coredump :TWEAKS:
/Documentation of disabling systemd coredump/
* OUTL [#A] Running Tasks in Parallel
:PROPERTIES:
...
* OUTL [#A] Chroot from live USB
/How to chroot from live USB stick/
* OUTL [#A] Arch Linux hibernation
:PROPERTIES:
...
* OUTL [#A] Notification Forwarding :subNotification:
...
* OUTL [#A] Run Service on Network Change :INTERNET:
:PROPERTIES:
...
* OUTL [#A] Systemd power supply detection :subSystemd:
...
* OUTL [#A] Advanced Suspend Mode :PERFORMANCE:
...
* OUTL [#A] Podman Quadlet :subPodman:containerization:
...
* OUTL [#A] Switching to Wayland Guide :subWayland:GUIDE:
:PROPERTIES:
...
* OUTL [#A] Archlinux Security :ARCHLINUX:Security:
...
#+END_SRC
*** Configuring Fail2ban for SSH :subSsh:
...
3.  Start =fail2ban= systemd service:
    ...
    #+END_SRC
* DOIN [#A] Homelab Remote Access Via VPN :subVPN:subWireGuard:subHomelab:
...
-  Gateway Peer
* OUTL [#A] Screen-saver and Screen-lock :subWayland:MONITOR:
/Lock and turn off screen after X value/ \\
/Visit:/  [[id:42f1f2f3-7ced-4803-b2af-74714bc55dc1][Wayland Screen Timeout @Org]]
* DONE Linux Hard vs Soft Link :subLinux@Structure@Feature:
CLOSED: [2026-06-21 Sun 20:05]
:PROPERTIES:
:ID: 9af75cf0-05ad-4828-abf7-cc327f027b84
:END:

#+name: Description
#+begin_quote org
In Linux, the primary difference between a soft link and a hard link is what the link points to:
- A soft link /(symbolic link or symlink)/ is points to a =filename=, similar to desktop shortcut
- A hard link points directly to the =inode= /(the actual data on the disk)/
#+end_quote


#+name: Note Admonition - Hardlink limitation
#+begin_quote org
[!NOTE]
