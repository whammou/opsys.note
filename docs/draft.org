...
*** VPN protocols
#+BEGIN_QUOTE
Internet Protocol Security (IPSec) is a set of protocols that supports secure commincation over Internet Protocol (IP) networkds through authentication and encryption. Its primary purpose is to ensure date integrity, data confidentiality, and data origin authentication between two communication parties
#+END_QUOTE
