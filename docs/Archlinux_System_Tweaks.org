...
#+FILETAGS: :typDoc:subProject:OPSYS:ARCH:TWEAKS:
...
* DONE [#A] Screen Time Event :ARCH:DPMS:
...
* DONE [#A] Disable fallback initramfs generation :PERFORMANCE:
...
* DONE [#A] User Service Proxy :subSystemd:opService:
...
- Create a proxy service that runs when a system-service started and signal the =user-service= to run on demand
  ...
  + To start the proxy-service run:
    ...
    #+END_SRC
* DONE [#A] Mounting USB Storage :DRIVE:
...
* DONE [#A] System Swap and Swappiness :ARCH:SWAP:
...
#+BEGIN_QUOTE org
[!WARNING]
There are some limitations of the implementation in `btrfs` and linux swap subsystem. Swap files on file systems spanning multiple devices are not supported. Data profile must be single
...
* DONE [#A] Disable Internet on Sleep :subSystemd:
CLOSED: [2026-04-12 Sun 17:02]
:PROPERTIES:
:ID: b3f0627d-14ce-4edc-9fe4-7b05f842c12d
:END:

#+name: Change Sleep Mode
#+begin_quote org
[!NOTE]
There are different sleep method supported on [[https://wiki.archlinux.org/title/Power_management/Suspend_and_hibernate][Power Management methods @Archwiki]]
#+end_quote

Currently using =deep= method to ensure internet is turned off on =sleep=
** DONE Advertise Internet Service :subSystemd@Config:
CLOSED: [2026-04-12 Sun 17:02]
:PROPERTIES:
:ID: b26e865d-fdc1-4539-9715-f90fef085fbc
:END:
...
#+name: /etc/NetworkManager/dispatcher.d/10-homelab-dispatcher.sh
#+begin_src sh
#!/usr/bin/env bash
...
#+name: Networkmanager failed to turn off gracfully
#+begin_quote markdown
[!WARNING]
...
#+name: Suspend-to-hibernate Override
#+begin_src systemd
[Service]
ExecStartPre=/usr/sbin/systemctl --user --machine=whammou@.host stop internet.target
#+end_src
